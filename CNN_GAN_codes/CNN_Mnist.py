...
def CNN_baseline_model():
	...
	cnn_model.add(Conv2D(32, (5, 5), input_shape=(1, 28, 28), activation='relu'))
...
# build the model
cnn_model = CNN_baseline_model()
# Fit the model
# verbose 2, reduce the output to one line for each training epoch (5 times), updates per 500 images
cnn_model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=5, batch_size=500, verbose=2)
...
# Fit the model
# verbose 2, reduce the output to one line for each training epoch (5 times), updates per 500 images
cnn_model_2.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=15, batch_size=200, verbose=2)
# Final evaluation of the model
cnn_scores_2 = cnn_model_2.evaluate(X_test, y_test, verbose=0)
print (cnn_scores_2)
print(cnn_model_2.metrics_names[1],": %.2f%%" % (cnn_scores_2[1]*100))
